...
import { UserPlus, Users, LogOut, X, Check, TrendingUp, BarChart3, User } from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  /* 1️⃣  Auto-hide success messages */
  useEffect(() => {
  ...
  /* 2️⃣  Prevent body scroll when modal is open */
  useEffect(() => {
    if (showCreateForm) {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      try {
        await API.get('/admin/welcome-admin', { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setSalesUserData((prev) => ({ ...prev, [name]: value }));
  ...
  const handleCreateSalesUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage({ type: 'error', text: error.response?.data?.message || 'Failed to create sales user' });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6 lg:py-8">
        ...
        {message && (
          <div
            className={`mb-6 p-3 sm:p-4 rounded-lg flex items-center space-x-3 ${
              message.type === 'success'
                ? 'bg-emerald-50 border border-emerald-200 text-emerald-800'
                : 'bg-red-50 border border-red-200 text-red-800'
            }`}
          >
            {message.type === 'success' ? <Check className="w-4 h-4 sm:w-5 sm:h-5" /> : <X className="w-4 h-4 sm:w-5 sm:h-5" />}
        ...
        {showCreateForm && (
          ...
          >
            <div
              className="bg-white/95 backdrop-blur-sm rounded-2xl p-4 sm:p-6 lg:p-8 w-full max-w-md -m-4 shadow-2xl border border-slate-200"
              ...
              <form onSubmit={handleCreateSalesUser} className="space-y-4 sm:space-y-6">
                <div>
                  ...
                  <input
                    ...
                    className={`w-full px-3 sm:px-4 py-2 sm:py-3 bg-white border rounded-lg text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 transition-all duration-200 text-sm sm:text-base ${
                      errors.name ? 'border-red-400 focus:ring-red-400' : 'border-slate-200 focus:ring-blue-400 focus:border-blue-400'
                    }`}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={handleInputChange}
                    className={`w-full px-3 sm:px-4 py-2 sm:py-3 bg-white border rounded-lg text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 transition-all duration-200 text-sm sm:text-base ${
                      errors.email ? 'border-red-400 focus:ring-red-400' : 'border-slate-200 focus:ring-blue-400 focus:border-blue-400'
                    }`}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={handleInputChange}
                    className={`w-full px-3 sm:px-4 py-2 sm:py-3 bg-white border rounded-lg text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 transition-all duration-200 text-sm sm:text-base ${
                      errors.password ? 'border-red-400 focus:ring-red-400' : 'border-slate-200 focus:ring-blue-400 focus:border-blue-400'
                    }`}
                ...
                <button
                  type="submit"
                  disabled={isLoading}
                  className="w-full py-2 sm:py-3 px-3 sm:px-4 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white rounded-lg font-medium transition-all duration-200 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none shadow-lg text-sm sm:text-base cursor-pointer"
                >
                  {isLoading ? (
